...
public class MySQLDatabaseManager implements DatabaseManager {
    ...
    public boolean createTables() {
        try (Connection conn = getConnection()) {
            ...
            System.out.println("✅ Tablas MySQL creadas correctamente (todas las tablas creadas)");
            return true;
            
        } catch (SQLException e) {
            String error = e.getMessage();
            if (error.contains("Connection refused") || error.contains("Communications link failure")) {
                System.err.println("❌ MySQL no está disponible (Docker no iniciado?)");
            } else if (error.contains("Access denied")) {
                System.err.println("❌ Error de acceso a MySQL: " + error);
            } else {
                System.err.println("❌ Error creando tablas MySQL: " + error);
            }
            return false;
        }
    }
    ...
    public boolean insertTestData() {
        try (Connection conn = getConnection()) {
            ...
            rs.next();
            if (rs.getInt(1) > 0) {
                System.out.println("ℹ️ Los datos de prueba ya existen en MySQL.");
                return true;
            }
            ...
            conn.commit();
            System.out.println("✅ Datos de prueba insertados correctamente en MySQL:");
            ...
            if (error.contains("Connection refused") || error.contains("Communications link failure")) {
                ...
                System.err.println("❌ Error insertando datos de prueba en MySQL: " + error);
                e.printStackTrace();
            }
            return false;
        }
    ...
    public boolean initialize() {
        System.out.println("🔄 Inicializando MySQL Database...");
        ...
        System.out.println("✅ MySQL Database inicializada correctamente");
        return true;
    }
    
    @Override
    public String getName() {
        return "MySQL Database (Docker)";
    }
}